...
const MIN_EVENT_INTERVAL: float = 300.0 # 5 minutes
const MAX_EVENT_INTERVAL: float = 600.0 # 10 minutes
...
func _execute_event(event: Dictionary) -> void:
	"""Execute an event's effects"""
	var category = event.get("category", "environmental")
